...
__global__ void Block_FW_phase1(int n, int r, int *global_D)
{
    ...
    // const int offset = B * r;

    // shared_D[y][x] = global_D[offset*(n+1) + y*n + x];
    // shared_D[y + HALF_B][x] = global_D[offset*(n+1) + (y+HALF_B)*n + x];
    // shared_D[y][x+HALF_B] = global_D[offset*(n+1) + y*n + (x+HALF_B)];
    // shared_D[y+HALF_B][x+HALF_B] = global_D[offset*(n+1) + (y+HALF_B)*n + (x+HALF_B)];
    // __syncthreads();

    // for(int k=0; k<B; k++)
    // {
    //     shared_D[y][x] = (shared_D[y][x] > shared_D[y][k] + shared_D[k][x])?(shared_D[y][k] + shared_D[k][x]):shared_D[y][x];
    //     shared_D[y + HALF_B][x] = (shared_D[y + HALF_B][x] > shared_D[y + HALF_B][k] + shared_D[k][x])?(shared_D[y + HALF_B][k] + shared_D[k][x]):shared_D[y + HALF_B][x];
    //     shared_D[y][x+HALF_B] = (shared_D[y][x+HALF_B] > shared_D[y][k] + shared_D[k][x+HALF_B])?(shared_D[y][k] + shared_D[k][x+HALF_B]):shared_D[y][x+HALF_B];
    //     shared_D[y + HALF_B][x+HALF_B] = (shared_D[y + HALF_B][x+HALF_B] > shared_D[y + HALF_B][k] + shared_D[k][x+HALF_B])?(shared_D[y + HALF_B][k] + shared_D[k][x+HALF_B]):shared_D[y + HALF_B][x+HALF_B];
    //     __syncthreads();
    // }

    // global_D[offset*(n+1) + y*n + x] = shared_D[y][x];
    // global_D[offset*(n+1) + (y+HALF_B)*n + x] = shared_D[y + HALF_B][x];
    // global_D[offset*(n+1) + y*n + (x+HALF_B)] = shared_D[y][x+HALF_B];
    // global_D[offset*(n+1) + (y+HALF_B)*n + (x+HALF_B)] = shared_D[y+HALF_B][x+HALF_B];
    __shared__ int shared_D[B][B];
...
__global__ void Block_FW_phase3(int n, int r, int *global_D)
{
    ...
    const int base_r = r * B * n + j * B;//D[r,i]
    ...
    for(int k = 0; k < B; k++)
    {
        // shared_D[y][x] = (shared_D[y][x] > col[y][k] + row[k][x]) ? (col[y][k] + row[k][x]) : shared_D[y][x];
        // shared_D[y + HALF_B][x] = (shared_D[y + HALF_B][x] > col[y + HALF_B][k] + row[k][x]) ? (col[y + HALF_B][k] + row[k][x]) : shared_D[y + HALF_B][x];
        // shared_D[y][x + HALF_B] = (shared_D[y][x + HALF_B] > col[y][k] + row[k][x + HALF_B]) ? (col[y][k] + row[k][x + HALF_B]) : shared_D[y][x + HALF_B];
        // shared_D[y + HALF_B][x + HALF_B] = (shared_D[y + HALF_B][x + HALF_B] > col[y + HALF_B][k] + row[k][x + HALF_B]) ? (col[y + HALF_B][k] + row[k][x + HALF_B]) : shared_D[y + HALF_B][x + HALF_B];
