...
double comm_time;
clock_t s_mpi, e_mpi;
void HybridConvolution(std::vector<float> &Dist, std::vector<float> &Mask, std::vector<float> &Result, int rank, int size)
{
...
int main(int argc, char **argv)
{
   ...
   // Input data
    ...
    start = clock();
    if(size > 1)
    HybridConvolution(Dist, Mask, Result, rank, size);
    else
