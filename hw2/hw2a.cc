...
{
    int j;
    // int boundary1 = width % 8;
    // int boundary2 = width - boundary1;
    // int boundary = (boundary1 == 0) ? width : boundary2;
    ...
    while (true)
    {
        ...
        //__m128d y0_sum = _mm_set_pd(y0, y0);
        __m512d y0_sum = _mm512_set1_pd(y0);

        /*yu--add*/
        // int boundary = ((width & 1) == 0) ? width : width - 1; // check is even or odd
        // int boundary = (width % 8 == 0) ? width : width - (width % 8);
        ...
        for (int i = 0; i < boundary; i += 16)
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                image[j * width + i + k] = repeat_a[k];
                image[j * width + i + 8 + k] = repeat_b[k];
            }
        }
...
int main(int argc, char **argv)
{
    ...
    /*create mutex lock for calculating working row for each thread*/
